...
LABELS = {
    ...
    "ASSISTANT_MANAGER": "ASSISTANT MANAGER", "SCOUT": "SCOUT",
    "YOUTH_TEAM_MANAGER": "YOUTH TEAM MANAGER", "YOUTH_TEAM_SCOUT": "YOUTH TEAM SCOUT",
...
def detect_bars(a: np.ndarray) -> list[tuple[int, int, int, int]]:
    ...
    for cx0, cx1 in ((76, 320), (334, 596)):
        ...
        for y in range(70, 472):
            ...
            wide = idx.size and (idx.max() - idx.min() >= 90) and \
                seg[idx.min():idx.max() + 1].mean() >= 0.45
            if wide:
...
def main() -> int:
    ...
    for key, s in SLOTS.items():
        ...
        # amount right-edge measured off the frame (differs per column AND panel):
        #   training L 335 / training R 592 ; role L 362 ; role R (mirror) 400.
        if s["kind"] == "train":
            wage_amount_right = 615 if bx >= 320 else 345
        elif s["mirror"]:
            wage_amount_right = 402        # role R (mirror): WAGE left of the bar, near centre
        else:
            wage_amount_right = 340        # role L: WAGE right of the bar (clear of SIGN/SACK)
        slots[key] = {
            "label": LABELS[key],
            "kind": s["kind"], "mirror": s["mirror"],
            "bar": [bx, by, bw, bh],
            "bar_color": bar_color,
            "name_x": bx + 6, "name_y": by,
            "stars_right": bx + bw - 4, "stars_y": by + 1,
            "wage_amount_right": wage_amount_right, "wage_amount_y": by + 10,
        }
    ...
    if os.environ.get("PM98_STAFF_DEBUG"):
        ...
        for key, s in slots.items():
            ...
            dd.rectangle([s["wage_amount_right"] - 58, s["wage_amount_y"],
                          s["wage_amount_right"], s["wage_amount_y"] + 9], outline=(255, 0, 255))
