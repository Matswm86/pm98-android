#!/usr/bin/env bash
# Oracle for FUN_005ae4c0 (1131 B, __fastcall this=player): one of the 3 "kick resolution" action
# handlers (case 0x14/0x16). Launches the ball toward the goal mouth: it computes the launch SPEED from
...
OUT=$SPECDIR/ae4c0_oracle.txt
SPEC=$SPECDIR/_ae4c0_run.spec
ROUT=$SPECDIR/_ae4c0_run.out
LUT=$SPECDIR/_ae4c0_lut.txt
...
READS=(
  ...
  "0x00260462 4"  # match+0x462 |= 0x40  (low byte)
...
BASE="$(poke 0x23002c 8);$(poke 0x230030 0);$(poke 0x23007c 1);$(poke 0x270080 1)"
...
: > "$OUT"
echo "# Oracle FUN_005ae4c0 (kick resolution, case 0x14/0x16). this=player. FUN_005ab5a0/590f00/4e9940 stubbed." >> "$OUT"
