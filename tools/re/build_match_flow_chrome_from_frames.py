...
def band_row_fill(a: np.ndarray, y0: int, y1: int, x0: int, x1: int,
                  is_band) -> None:
...
def main() -> None:
    ...
    spec["match_options"] = {
        ...
        "view_btns": {  # measured left/right edges -> x,w
            "watch": [116, 97], "highlights": [220, 97],
            "brief": [325, 97], "results": [428, 97],
        },
    ...
    for frame, name in [(load(F_FT), "fulltime"), (load(F_HT), "halftime")]:
        ...
        expect(220 <= tx0 <= 260 and 340 <= tx1 <= 380,
               f"title {name} bbox x {tx0}..{tx1}")
...
COLS = {                      # cell columns (x span); scrollbar to the right of each
    "hb": (19, 144), "hg": (169, 289), "ab": (332, 457), "ag": (482, 602),
}
...
def _rowmed(a: np.ndarray, f: np.ndarray, y0: int, y1: int, xa: int, xb: int,
            sx0: int, sx1: int, keep: tuple | None = None) -> None:
...
def _bake_result(f: np.ndarray, out: str, full: bool) -> dict:
    ...
    expect(abs(px(f, 540, 210)[1] - 223) < 30 and px(f, 540, 210)[0] > 140,
           "away GOALS green cell")
    ...
    # STADIUM body: per-row median (from the far-right bg strip) removes the name +
    # every label + value. Two zones so the car icon (top-left) is kept: the name row
    # (right of the car) and the label rows (below the car, full width). The screen
    # redraws the ground name + CAPACITY / ATTENDANCE; the money rows stay blank.
    # (TOTAL FOULS values are over-painted by the screen — honest absent.)
    _rowmed(a, f, 356, 376, 78, STAD[2] - 2, 268, 286)     # ground-name row
    _rowmed(a, f, 376, 454, 18, STAD[2] - 2, 268, 286)     # label rows (below car)

    # MAN OF THE MATCH (full time): clear the mug + name band to its blue (keep the
    # "MAN OF THE MATCH" header + the generic player sprite).
    if full:
        a[374:414, MOTM[0] + 4:602] = np.array([0, 0, 160], dtype="uint8")

    save(a, out)
    return {
        "binding_frame": F_FT.name if full else F_HT.name,
        "scoreband_y": list(SB_Y), "box_l": list(BOX_L), "box_r": list(BOX_R),
        "name_home": list(NAME_H), "name_away": list(NAME_A),
        "band_fill": list(BAND_FILL), "box_fill": list(BOX_FILL),
        "poss": list(POSS), "poss_pill": list(POSS_PILL),
        "cols": {k: list(v) for k, v in COLS.items()},
        "row_y0": ROW_Y0, "row_pitch": ROW_PITCH, "row_h": ROW_H, "n_rows": N_ROWS,
        "fouls_y": list(FOULS_Y),
        "colors": {
            "scoreband": list(BAND_FILL), "box": list(BOX_FILL),
            "cell_green": list(px(f, 540, 210)), "cell_blue": list(px(f, 80, 210)),
            "stadium_row": list(STAD_BODY_BG),
        },
        # scoreband kit anchors (home top-left / away top-right, club escudo overlay)
        "kit_home_xy": [6, 60], "kit_away_xy": [590, 60], "kit_wh": [46, 52],
        # continue button (full time) hit rect
        "continue": [479, 439, 112, 25],
        # stadium panel body: label rows (redrawn by the screen). y-rows measured.
        "stadium_panel": list(STAD), "stadium_icon_x": STAD_ICON_X,
        "stadium_body_bg": list(STAD_BODY_BG),
        "stadium_rows_y": [357, 380, 396, 412, 428],   # name, cap, att, boards, sponsor
...
def _bake_brief() -> dict:
    ...
    return {
        ...
        "clock": list(BR_CLOCK), "state_y": list(BR_STATE_Y),
        "band_y": list(BR_BAND_Y), "name_home": list(BR_NAME_H), "name_away": list(BR_NAME_A),
        "box_l": list(BR_BOX_L), "box_r": list(BR_BOX_R),
        "kit_home": list(BR_KIT_H), "kit_away": list(BR_KIT_A), "events": list(BR_EVENTS),
        # right-column + bottom in-match buttons (frame-measured, x,y,w,h)
        "btn_lineup": [495, 227, 133, 33], "btn_tactics": [495, 283, 133, 33],
        "btn_mtm": [495, 339, 133, 33], "btn_stats_r": [495, 393, 133, 30],
        "btn_stats_l": [14, 393, 133, 30], "btn_kick": [262, 442, 156, 30],
        "btn_exit": [508, 442, 120, 30],
        "note": ("Frame 073 is KICK OFF (empty EVENTS, 0-0): the resting chrome is "
