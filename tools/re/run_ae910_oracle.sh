#!/usr/bin/env bash
# Oracle for FUN_005ae910 (1131 B, __fastcall this=player): one of the 3 "kick resolution" action
# handlers (case 0x15). Launches the ball toward the goal mouth: it computes the launch SPEED from
...
OUT=$SPECDIR/ae910_oracle.txt
SPEC=$SPECDIR/_ae910_run.spec
ROUT=$SPECDIR/_ae910_run.out
LUT=$SPECDIR/_ae910_lut.txt
...
READS=(
  ...
  "0x00270004 4"  # ball+0x4  += player+0x20
  "0x00270008 4"  # ball+0x8  += player+0x24
  "0x0027000c 4"  # ball+0xc  += player+0x28
  "0x00270064 4"  # ball+0x64 = 1 (set64)
...
BASE="$(poke 0x23002c 5);$(poke 0x230030 0);$(poke 0x23007c 1);$(poke 0x270080 1)"
...
BASE="$BASE;$(poke 0x230020 0x11111);$(poke 0x230024 0x22222);$(poke 0x230028 0x33333)"  # player vel (added to ball pos)
BASE="$BASE;$(poke 0x270004 0x1000);$(poke 0x270008 0x2000);$(poke 0x27000c 0x3000)"  # ball pos base
...
: > "$OUT"
echo "# Oracle FUN_005ae910 (kick resolution, case 0x15). this=player. FUN_005ab5a0/590f00/4e9940 stubbed." >> "$OUT"
