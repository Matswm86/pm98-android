...
the **u16 LE immediately before the player's name** — i.e. at `name_start − 3` (one pad
byte sits between the id and the `[u16 len]` name prefix). Equivalently it is the field
that trails the *previous* player's attribute block (`6c 6b` + 10 attrs + `01`), since
...
listed first). The field belongs to the player whose **name it precedes**. `name_start −
3` is the single rule that works for every player, including the first (its id sits just
after the record header) and the last (whose own trailing field is the `00` terminator,
not a photo).
