...
| `DBDAT/EQUIPOS.PKF` | RAR+ISO | 4,099,004 | 476 | **476 per-team `.DBC` database records** — incl. each club's OWN tactic (11x8-u16 slot block + 7 lever bytes, `FUN_00579c70`) AND its SHIPPED XI (squad records' player+0x1b slot bytes, `FUN_005820f0`; both decoded 2026-07-06 → `club_tactics_re.md`, exported to `app/data/club_tactics.json`; extended-record tail T4..T9 bio pages + T10 career CSV exported verbatim to `assets/bios.json`, 2025 players, no app consumer yet — display surface un-walked) |
