...
📦 **[Download the latest APK](https://github.com/Matswm86/pm98-android/releases/download/latest/pm98-4fbfc87.apk)** — one tap downloads `pm98-4fbfc87.apk` (current build) straight to your phone.

> **This build (2026-07-24)** fixes the **GROUND** screen's stadium picture. The 12 aerial-view tiles were 256-column wrapped — a hard seam ran down the middle of every one, and the panel rendered at 4% pixel-exact against the real game. Solved against a 1:1 capture of the original: the picture is now **98% pixel-exact** and the whole GROUND screen went from 55% to 78%. Text on the **alert box, the DATABASE player card and the tactics board** also renders again — those screens read the BMFont atlas back off disk, which an exported APK does not carry, so they now go through a shipped metrics table instead. Behind the scenes, the match engine's parity frontier turned out to be a measurement artefact rather than a real divergence: all 22 players and the ball match the original tick for tick from clock 270 to 660 ([`docs/re/M5_S55_SAMPLING_PHASE_ARTEFACT.md`](docs/re/M5_S55_SAMPLING_PHASE_ARTEFACT.md)).
