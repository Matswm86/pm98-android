...
📦 **[Download the latest APK](https://github.com/Matswm86/pm98-android/releases/download/latest/pm98-79b6ec8.apk)** — one tap downloads `pm98-79b6ec8.apk` (current build) straight to your phone.

> **This build (2026-07-24)** adds the two **MONTHLY AWARD** sheets — *MANAGERS OF THE MONTH* and *PLAYERS OF THE MONTH* — rebuilt from the real game's own frames and raised at the end of every calendar month, exactly where the original raises them ([`docs/re/awards_screens_re.md`](docs/re/awards_screens_re.md)). The **FULL TIME** board is repaired too: its stadium panel had lost all five row labels and its arrow chevrons to an over-eager bake, and now prints CAPACITY, ATTENDANCE, ATTENDANCE MONEY, SPONSOR BOARDS SOLD and SPONSORSHIP MONEY on the original's own pixels — every value lands on the frame's exact ink position. **MAN OF THE MATCH** is live for the first time, named and pictured, using the game's own ported selector. And the **alert box now shows its text on the phone**: the fix shipped in the previous build is confirmed present in this APK, so signings, rejections and "The scout has finished his search." finally read as words instead of a blank white rectangle.
