...
var _staff: Array = []                # accepted for API parity; the per-skill staff band is a GAP
...
func _on_input(e: InputEvent) -> void:
	...
	match was:
		...
		"auto":
			# GAP: PM98's AUTO assigns a per-player training FOCUS; the app has no
			# focus model, so AUTO is a documented no-op (press feedback only).
			pass
		_:
...
func _draw() -> void:
	...
	# TOTAL TRAINABLE PLAYERS = squad size (frame 004: 25). Grid TOTAL = 0 (rest,
	# no focus assigned — the app assigns none).
	var ttp := str(_by_id.size())
	PMChrome.text(self, _f8, TTP_CELL[0], TTP_Y, ttp, C_TTP, 11, 1, float(TTP_CELL[1]))
	PMChrome.text(self, _f8, TOTAL_CELL[0], TOTAL_Y, "0", C_TOTAL, 11, 1, float(TOTAL_CELL[1]))
