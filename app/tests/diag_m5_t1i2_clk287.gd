...
## Run: ~/godot462 --headless --path app --script res://tests/diag_m5_t1i2_clk287.gd
...
func _run() -> void:
	...
	while t < TICK_CAP:
		...
		for s in Pm98Movement.steer_trace:
			if String(s[0]).begins_with("t1.i2"):
				strc += str(s)
		print("t=%3d clk=%3d pos=(%d,%d) 13c=%d 17c=%x face=%x yaw=%x spd68=%d cur6c=%d p54=%d p58=%d ph448=%d steer=%s" % [
			t, clk, _si(p, 4), _si(p, 8), _g(p, 0x13c), _si(p, 0x17c) & 0xffffffff,
			_g(p, 0x34) & 0xffff, _g(p, 0x64) & 0xffff, _si(p, 0x68), _si(p, 0x6c),
			_si(p, 0x54), _si(p, 0x58), _g(m, 0x448), strc])
		...
		for b in Pm98Movement.b0040_trace:
			var bd: Dictionary = b
			if String(bd.get("who", "")).begins_with("t1.i2"):
