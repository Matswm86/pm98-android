extends SceneTree
## Oracle-backed INTEGRATION parity test (case-0x13 GATE). Drives Pm98Action.engine_tick through the
## case-0x13 (keeper-distribution / kick windup) bVar17-TRUE arm -- FUN_005a4600's OWN teammate-search +
## ball-launch block -- and on into the nested leaf:
##   engine_tick (FUN_005a4600, action 0x13) -> [teammate search + ball launch] -> setup_shot
##     (FUN_005ac1a0) -> resolve_post_shot (FUN_005ab5a0)
## and asserts the full residue + rng draw count against the REAL chain run end-to-end under the Ghidra
## PCode emulator (tools/re/run_engine_dist_oracle.sh -> specs/engine_dist_oracle.txt; setup_shot run REAL,
## resolve_post_shot reached transitively + REAL). This gates the previously TRANSCRIPTION-ONLY case-0x13
## pre-block (+0xa0/a4/a8 aim + ball+4/8/c launch vector) AND the setup_shot wiring in
## Pm98Action._case_distribution. Companion to test_engine_cascade.gd (same leaf via the acc40/case-4 path).
##
## Run headless from the project dir:
##   ~/godot462 --headless --path app --script res://tests/test_engine_dist.gd
##
## NOTE: the fixture drives resolve_post_shot's to_tail path (match+0x438 == player) and a populated gs
## roster with one eligible teammate (so iVar16 != 0). The no-eligible-teammate edge (iStack_20 = -1
## leftover seeding pv2) is NOT exercised here -- see Pm98Action._case_distribution CAVEAT.
...
const READS := [
	...
	# case-0x13 pre-block residue (aim + ball launch vector + cleared b+0x4c)
	[0x2300a0, "p", 0xa0, 4], [0x2300a4, "p", 0xa4, 4], [0x2300a8, "p", 0xa8, 4], [0x2300b4, "p", 0xb4, 4],
	[0x270004, "ball", 0x4, 4], [0x270008, "ball", 0x8, 4], [0x27000c, "ball", 0xc, 4], [0x27004c, "ball", 0x4c, 4],
	...
	[0x270044, "ball", 0x44, 4], [0x270048, "ball", 0x48, 4],
...
func _init() -> void:
	var path := ProjectSettings.globalize_path("res://").path_join("../tools/re/specs/engine_dist_oracle.txt").simplify_path()
	var f := FileAccess.open(path, FileAccess.READ)
	if f == null:
		_ok(false, "engine dist oracle unreadable (run tools/re/run_engine_dist_oracle.sh)")
	else:
		var name := ""
...
func _check(name: String, mems: Dictionary) -> void:
	...
	# Pointer fields hold a player/struct Dict ref after the chain (engage writes ball+0x44/48/50 = P).
	...
	var addr_of := {0x230000: p, 0x270000: stores["ball"], 0x260000: stores["m"],
		0x280000: stores["gs"], 0x2b0000: stores["stat"], 0x2a0000: built["team"]}
...
## Mirror of run_engine_dist_oracle.sh (FRAME + SETUP + RESOLVE + PTRS + GSBASE + TEAM). Linked Dicts:
## p[0x18c]=match, p[0x190]=ball, p[0x184]=gs, p[0x3b8]=stat, ball[0x1d4]=match, match[0x468]=play-state,
## match[0x438]=p (resolve to_tail), gs[0]=[teammate] (the case-0x13 roster base; teammate+0x2bc != 0).
func _fixture() -> Dictionary:
	var team := {0x2bc: 1, 0x4: 0x1000000, 0x8: 0x80000, 0xc: 0}
	var stat := {}
	var playstate := {0xfa0: 0}
	var m := {
		# RESOLVE: match+0x460 = 0 (no stale taker), match+0x448 = 0 (open play)
		0x460: 0, 0x448: 0, 0x468: playstate,
		0x438: null,
	}
	# ball: NO 0x4c -- the case-0x13 block sets it to the found teammate (then resolve clears it).
	var ball := {0x1d4: m, 0x70: 100, 0x40: 0, 0x63: 0}
	var gs := {0: [team], 0x2ee: 0}
	var p := {0x18c: m, 0x190: ball, 0x184: gs, 0x3b8: stat}
	# FRAME: action 0x13, frame guard p+0x2c=5 / p+0x30=3 (->0 after tick_action with +0x48 locked) -> bVar17.
	p[0x40] = 0x13; p[0x2c] = 5; p[0x30] = 3; p[0x48] = 5; p[0x88] = 0
	p[0x3a4] = 0x100000; p[0x4] = 0; p[0x8] = 0; p[0xc] = 0; p[0x2bc] = 1
	p[0x58] = 6; p[0x34] = 0; p[0x5e] = 0
	# SETUP inputs (ball owned via the found teammate -> skill +0x394; touch +0x54/58; rating +0x70)
	p[0x394] = 80; p[0x3a0] = 70; p[0x70] = 8000; p[0x54] = 5
