extends SceneTree
## M5 s38: dump the FULL input + intermediate-term snapshot of _b0040_target for t1.i9
## per tick, so the port's (1647,2) arm-step steer target can be diffed INPUT-by-INPUT
## against silicon (12361,47217). Reads the SAME persisted frame-0 + seed as
...
func _run() -> void:
	...
	print("PORT t1.i9  _b0040_target input+term trace")
	var t := 0
	while t < TICK_CAP:
		...
		var clk := _g(m, 0x450)
		if t >= 24 and t <= 30:
			for row in Pm98Movement.b0040_trace:
				if String((row as Dictionary).get("who", "")).begins_with("t1.i9"):
					print("t=%2d clk=%3d act=%d spd=%d  %s" % [
						t, clk, _g(p, 0x40), _si(p, 0x68), JSON.stringify(row)])
		if clk >= 8:
			break
