extends SceneTree
## M5 s30: per-clk end-seed trace for LCG-parity diffing against a reference capture's
## per-clk seed table (capture2/timeline.jsonl or the wide-q capture). Prints "SEED clk state"
## once per clk transition. Run:
##   ~/godot462 --headless --path app --script res://tests/diag_m5_seedtrace.gd
...
func _run() -> void:
	...
	while t < TICK_CAP:
		...
		if clk != last_clk:
			if last_clk >= 0:
				print("SEED %d %d" % [last_clk, _last_seed])
			last_clk = clk
		_last_seed = rng.state
	print("SEED %d %d" % [last_clk, _last_seed])
