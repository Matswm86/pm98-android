extends SceneTree
## M5 live-drive follow-up (diag-only): print the PORT rng.state + ball-vel per tick from the
## SAME team-1 frame-0 (steertgt seed 0xc357aa2c) the silicon armwatch used, so the port's seed
...
func _run() -> void:
	...
	print("PORT kick-timing seed probe. frame0 seed=0x%08x  (silicon kick seed=0x041e701f @126 draws, etick27)" % rng.state)
	var t := 0
	var kicked := false
	while t < TICK_CAP:
		var seed_pre := rng.state
		...
		if not kicked and (vel_post[0] != 0 or vel_post[1] != 0):
			kicked = true
			print(">>> KICK at t=%d  clk=%d  seed_pre=0x%08x  seed_post=0x%08x  vel=%s" % [
				t, _g(m, 0x450), seed_pre, rng.state, str(vel_post)])
		if t <= 46:
			print("t=%2d clk=%d seed=0x%08x vel=%s" % [t, _g(m, 0x450), rng.state, str(vel_post)])
		t += 1
		if ret == 0:
			m[0x1a1e] = 1
		if _g(m, 0x450) >= 10:
